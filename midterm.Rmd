...
```{r}
library(rvest)
basicurl <- "https://www.imdb.com/search/title/?groups=top_1000&start=1"
basichtml <- read_html(basicurl)

rating <- html_nodes(basichtml, ".ratings-imdb-rating strong")
rating <- html_text(rating, trim = TRUE)

number_of_ratings <- html_nodes(html, ".lister-item-header a")
number_of_ratings <- html_text()

year <- html_nodes(basichtml, ".text-muted.unbold")
year <- html_text(year, trim = TRUE)

gross_revenue <- html_nodes(basichtml, ".ghost~ .text-muted+ span")
gross_revenue <- html_text(gross_revenue, trim = TRUE)

budget <- html_nodes(html, ".lister-item-header a")

title <- html_nodes(basichtml, ".lister-item-header a")
title <- html_text(titles, trim = TRUE)

genre <- html_nodes(basichtml, ".genre")
genre <- html_text(genre, trim = TRUE)

top_billed <- html_nodes(basichtml, ".lister-item-content .ghost~ a")
top_billed <- html_text(top_billed, trim = TRUE)

helpful <- html_nodes(html, ".lister-item-header a")

movie_data <- data.frame(rating, number_of_ratings, year, gross_revenue, budget, title, genre, top_billed, helpful)
```
